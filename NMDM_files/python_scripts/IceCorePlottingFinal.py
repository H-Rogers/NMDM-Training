...
def ice_core_plot_final(ice_age_deut, deut, deltaTS,ice_age_o18,o18,ice_age_CO2,CO2):
    # First set up the plot dimensions etc.
    plt.figure(1, facecolor='White',figsize=[8,10])
    # We are going to use subplots here.  We are going to specify the axes used
    # in each.  This will give us a bit more flexibility later on (bear with
    # me here)
    
    # Subplot a -> Temperature   
    ...
    ax1.axvline(x=130,color='black') 
    ax1.set_ylabel(u'$\Delta TS  (^{o}C)$',fontsize=10)
    ax1.grid(True)           
    ax1.annotate('a', xy=(0.05,0.9), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=rcParams['font.size']+2) 
    ax1.annotate('Panel (e)', xy=(65,-9), xycoords='data',color='white',horizontalalignment='center', verticalalignment='center', fontsize=rcParams['font.size']+2) 
    ax1.set_xlim(xmax=420)    
    ax1.set_title('420ka Climate Record from Vostok Ice Core (Petit et al., 1999)',fontsize=12) 
    ...
    ax2.annotate('b', xy=(0.05,0.9), xycoords='axes fraction',backgroundcolor='none',horizontalalignment='left', verticalalignment='top', fontsize=rcParams['font.size']+2) 
    ax2.set_xlim(xmax=420)
    ...
    ax3.annotate('c', xy=(0.05,0.9), xycoords='axes fraction',backgroundcolor='white',horizontalalignment='left', verticalalignment='top', fontsize=rcParams['font.size']+2) 
    ax3.set_xlim(xmax=420)    
    
    #Subplot d -> Oxygen isotopes
    ...
    ax4.annotate('d', xy=(0.05,0.9), xycoords='axes fraction',backgroundcolor='white',horizontalalignment='left', verticalalignment='top', fontsize=rcParams['font.size']+2) 
    ax4.set_xlim(xmax=420)
    
    # Layout adjustments    
    # Lets stack the subplots together so that we waste less space       
    xticklabels = ax1.get_xticklabels()+ax2.get_xticklabels()+ax3.get_xticklabels()
    plt.setp(xticklabels, visible=False)    
    plt.subplots_adjust(hspace=0.001)
    # Now the axis labels are overlapping at each end, which is far from ideal
    # so lets stick one of them on the right hand side instead
    ax2.yaxis.tick_right()
    ax2.yaxis.set_label_position("right")
    ax4.yaxis.tick_right()
    ax4.yaxis.set_label_position("right")
    
    # Subplot e -> temperature and CO2 for last glacial cycle    
    ...
    ax5.set_ylabel(u'$\Delta TS  (^{o}C)$',fontsize=10)
    ...
    majorLocator = tk.MultipleLocator(25)
    ax6.yaxis.set_major_locator(majorLocator) 
    
    # Adjust range of y axes to separate the datasets sufficiently
    ...
    ax5.annotate('Last Glacial Maximum', xy=(26.5, -7),  xycoords='data',xytext=(0.5, 0.7), textcoords='axes fraction',
                horizontalalignment='right', verticalalignment='top',fontsize = 10,
                arrowprops=dict(arrowstyle="fancy",facecolor="gold",edgecolor="black",linewidth=0.25,shrinkB=4,connectionstyle="arc3,rad=0.1",))
    plt.savefig("VostokIceCoreData.svg",format="svg")
    plt.show()
